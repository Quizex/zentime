...
async function applySelectOptionsColumns() {
  try {
    console.log('Applying select_options columns and updating zentime_replace_my_data function...');
    
    // 1. Add select_options column to zentime_categories if it doesn't exist
    ...
    // 2. Add select_options column to zentime_work_items if it doesn't exist
    const addWorkItemColumn = `
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1
    ...
    // 3. Add select_options column to zentime_events if it doesn't exist
    const addEventColumn = `
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1
    ...
    // 4. Update zentime_replace_my_data function
    const updateReplaceFunction = `
    ...
    // 执行 SQL 语句
    const statements = [addCategoryColumn, addWorkItemColumn, addEventColumn, updateReplaceFunction];
